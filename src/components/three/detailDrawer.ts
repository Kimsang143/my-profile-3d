...
export class DetailDrawer {
	...
	draw = (progress: number, progress2: number) => {
		...
		y = this._drawSubText('Name.', y)
		const nickname = `${this._work.name}`
		y = this._drawMainText(nickname, y)

		y = this._drawSubText('No.', y)
		...
		y = this._drawSubText('Date of Birth', y)
		...
		y = this._drawSubText('Technology/Skill', y)
		this._drawMainRect(y, progress, 1)
		text = progress === 0 ? this._work.technology : this._getRandomString(this._work.technology.length)
		y = this._drawMainText(text, y)
		

		// y = this._drawSubText('Framework.', y)
		// const framework = `${this._work.framework}`
		// y = this._drawMainText(framework, y)
